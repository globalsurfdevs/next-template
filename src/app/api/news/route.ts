    ...
    try {
        ...
        if (!id) {
            const { data, error } = await supabase
                .from('news')
                .insert([
                    { title, content, date:new Date(date), image: imagePath,metadataTitle,metadataDesc },
                ])
                .select()
            ...
            const { data, error } = await supabase
                .from('news')
                .update({ title, content, date:new Date(date), image: imagePath,metadataTitle,metadataDesc })
                .eq('id', id)
                .select()
